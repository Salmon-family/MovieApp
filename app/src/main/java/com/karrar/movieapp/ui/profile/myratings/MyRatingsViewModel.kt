...
class MyRatingsViewModel @Inject constructor(
    ...
    init {
        getRatedMovies()
        Log.i("MyRatingsViewModel", "MyRatingsViewModel: ${_ratedMovies.value.toString()}")
    }
    ...
    private fun getRatedMovies() {
        viewModelScope.launch {
            accountRepository.getSessionId().collect { sectionId ->
                wrapWithState({
                    val result = movieRepository.getRatedMovies(sectionId.toString())
                    _ratedMovies.postValue(UIState.Success(result))
                }, {
