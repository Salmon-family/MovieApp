package com.karrar.movieapp.domain.usecase.allMedia

import androidx.paging.Pager
import androidx.paging.PagingData
import androidx.paging.map
import com.thechance.repository.MovieRepository
import com.thechance.repository.SeriesRepository
import com.karrar.movieapp.domain.enums.AllMediaType
import com.karrar.movieapp.domain.mappers.movie.MovieMapper
import com.karrar.movieapp.domain.mappers.series.TVShowMapper
import com.karrar.movieapp.domain.models.Media
import com.karrar.movieapp.utilities.Constants
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.map
import javax.inject.Inject

class GetMediaByTypeUseCase @Inject constructor(
    private val movieRepository: MovieRepository,
    private val seriesRepository:  SeriesRepository,
    private val movieMapper: MovieMapper,
    private val tvShowMapper: TVShowMapper,
) {

    suspend operator fun invoke(type: AllMediaType, actorId: Int = 0): Flow<PagingData<Media>> {
        return when (type) {
            AllMediaType.ACTOR_MOVIES -> {
                wrap({ movieRepository.getActorMoviesPager(actorId) }, movieMapper::map)
            }
            AllMediaType.LATEST,
            AllMediaType.AIRING_TODAY,
            -> {
                wrap(seriesRepository::getAiringTodayTvShowPager, tvShowMapper::map)
            }
            AllMediaType.ON_THE_AIR -> {
                wrap(seriesRepository::getTopRatedTvShowPager, tvShowMapper::map)
            }
            AllMediaType.POPULAR -> {
                wrap(seriesRepository::getPopularTvShowPager, tvShowMapper::map)
            }
            AllMediaType.TOP_RATED -> {
                wrap(seriesRepository::getTopRatedTvShowPager, tvShowMapper::map)
            }
            AllMediaType.TRENDING -> {
                wrap(movieRepository::getTrendingMoviesPager, movieMapper::map)
            }
            AllMediaType.NOW_STREAMING -> {
                wrap(movieRepository::getNowPlayingMoviesPager, movieMapper::map)
            }
            AllMediaType.UPCOMING -> {
                wrap(movieRepository::getUpcomingMoviesPager, movieMapper::map)
            }
            AllMediaType.MYSTERY -> {
                wrap({ movieRepository.getMovieByGenre(Constants.MYSTERY_ID) }, movieMapper::map)

            }
            AllMediaType.ADVENTURE -> {
                wrap(
                    { movieRepository.getMovieByGenre(Constants.ADVENTURE_ID) },
                    movieMapper::map
                )
            }
        }
    }

    private suspend fun <T : Any> wrap(
        data: suspend () -> Pager<Int, T>,
        mapper: (T) -> Media,
    ): Flow<PagingData<Media>> {
        return data().flow.map { pager -> pager.map { mapper(it) } }
    }
}